...
Options:
    ...
    --model-path=<file>                     model file [default: model.bin]
    ...
    --valid-niter=<int>                     perform validation after how many iterations [default: 1250]
    ...
    --aspect-active=<bool>                  aspect repeat forbidden [default: False]
    --trichar-repeat-active=<bool>          trichar repeat forbidden [default: True]

    --sample_size=<int>                     sample size [default: 8]
    --raml_sample_mode=<str>                raml sample mode [default: hamming_distance_impt_sample]
    --raml_bias_groundtruth=<bool>          make sure ground truth y* is in samples [default: True]
...
class NMT(nn.Module):
    ...
    def forward(self, src_sents, tgt_sents, img_vecs):
        """
        ...
        if self.copy and self.coverage:
            ...
            h_ts, ctx_ts, alpha_ts, att_vecs, tgt_word_embeds = self.decode(src_encodings, src_sent_masks, decoder_init_vec, tgt_sents_var[:-1])
    ...
    def decode(self, src_encodings, src_sent_masks, decoder_init_vec, tgt_sents_var):
        ...
        for y_tm1_embed in tgt_word_embeds.split(split_size=1):
            ...
            if self.coverage:
                ...
                (h_t, cell_t), ctx_t, alpha_t, att_t = self.step(x, h_tm1, src_encodings, src_encoding_att_linear, src_sent_masks, None)
    ...
    def dot_prod_attention(self, h_t, src_encoding, src_encoding_att_linear, mask, att_history):
        ...
        if self.coverage and att_history is not None:
            att_hidden_text = torch.tanh(self.att_ht_linear(h_t).unsqueeze(1).expand_as(src_encoding_att_linear) +
                                     att_history.unsqueeze(2).expand_as(src_encoding_att_linear) + src_encoding_att_linear)
        else:
    ...
    def beam_search(self, src_sent, img, aspect_sent, first_uni2id, first_bi2id, tri2id, beam_size, max_decoding_time_step,  aspect_map, args):
        """
...
def aspect_precision(ref_word_list, sampled_word_list, aspect_dict):
    golden_aspects = set()
    sampled_aspects = set()
    for w in ref_word_list:
        if w in aspect_dict:
            golden_aspects.add(aspect_dict[w])
    for w in sampled_word_list:
        if w in aspect_dict:
            sampled_aspects.add(aspect_dict[w])

    if sampled_aspects:
        return 1.0 * len(golden_aspects.intersection(sampled_aspects)) / len(sampled_aspects)
    return 0

def str_to_bool(str):
    return True if str.lower() == 'true' or str == '1' else False

def evaluate_ppl(model, dev_data, coverage, batch_size=32):
    """
    ...
    with torch.no_grad():
        ...
        for src_sents, tgt_sents, imgs, asps in batch_iter(dev_data_unfold, batch_size):
            if coverage:
                loss, coverage_loss = model(src_sents, tgt_sents, imgs)
                loss = -loss.sum()
            else:
                loss = -model(src_sents, tgt_sents, imgs).sum()
...
def train(args):
    ...
    train_data = list(zip(train_data_src, train_data_tgt, train_data_img))
    dev_data = list(zip(dev_data_src, dev_data_tgt, dev_data_img))
    ...
    while True:
        ...
        for src_sents, tgt_sents, img_vecs in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            train_iter += 1
            optimizer.zero_grad()
            batch_size = len(src_sents)
            if coverage:
                example_losses, coverage_losses = model(src_sents, tgt_sents, img_vecs)
                example_losses = -example_losses
            else:
                example_losses = -model(src_sents, tgt_sents, img_vecs)
            ...
            if train_iter % valid_niter == 0:
                ...
                dev_hyps = beam_search_dev(model,
                            dev_data_src, dev_data_img, dev_data_aspect,
                            beam_size=10,
...
def train_raml(args):
    aspect_data_map, aspect_data_map_reverse = read_aspect_file(args['--aspect-file'])
    tau = 0.85
    ...
    payoff_prob, Z_qs = generate_hamming_distance_payoff_distribution(max(len(sent) for sent in train_data_tgt), vocab_size=len(vocab.tgt) - 3, tau=tau)

    print('begin RAML training')

    while True:
        epoch += 1
        for src_sents, tgt_sents, img_vecs, aspects in batch_iter(train_data, batch_size=train_batch_size, shuffle=True):
            train_iter += 1
            ...
            for src_sent, tgt_sent, img_vec, aspect in zip(src_sents, tgt_sents, img_vecs, aspects):
                ...
                e_samples = np.random.choice(range(tgt_sent_len + 1), p=payoff_prob[tgt_sent_len], size=int(args['--sample_size']), replace=True)
                ...
                for i, e in enumerate(e_samples):
                    ...
                    if args['--raml_sample_mode'] == 'hamming_distance_impt_sample':
                        if e > 0:
                            ...
                            bleu_score = aspect_precision(tgt_ref_tokens, new_tgt_sent[1:-1], aspect_data_map_reverse[aspect])
                ...
                raml_img_vecs.extend([img_vec] * len(tgt_samples))
            ...
            # (batch_size,)
            scores = model(raml_src_sents, raml_tgt_sents, raml_img_vecs)

            weighted_loss = -scores * weights_var
            ...
            batch_losses_val = weighted_loss.item()
            report_loss += batch_losses_val
            cum_loss += batch_losses_val
            ...
            if train_iter % valid_niter == 0:
                ...
                print('validation: iter %d, dev. ROUGE2 %f' % (train_iter, dev_rouge2), file=sys.stderr)
...
def beam_search_dev(model, test_data_src, test_data_img, test_data_aspect, beam_size, max_decoding_time_step, aspect_map, args):
    ...
    with torch.no_grad():
        for src_sent, img, aspect_sent in tqdm(test_data_tuple, desc='Decoding', file=sys.stdout):
            example_hyps = model.beam_search(src_sent, img, aspect_sent, first_uni2id, first_bi2id, tri2id, beam_size=beam_size, max_decoding_time_step=max_decoding_time_step, aspect_map=aspect_map, args=args)
